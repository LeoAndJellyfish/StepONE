...
示例：python package.py 1.0.0
       python package.py 1.0.0 --platforms=macos
       python package.py 1.0.0 --platforms=windows,android,macos
"""
...
def pause_if_interactive(message="按回车键退出..."):
    """仅在交互式模式下暂停"""
    if is_interactive():
        try:
            input(f"\n{message}")
...
def package_windows(project_root, version_dir, version):
    ...
    package_name = f"StepONE_v{version}_windows_portable"
    package_dir = version_dir / package_name

    if not (win_source / "stepone_app.exe").exists():
...
def package_macos(project_root, version_dir, version):
    ...
    app_name = "StepONE.app"
    package_name = f"StepONE_v{version}_macos"
    package_dir = version_dir / package_name
...
def package_linux(project_root, version_dir, version):
    ...
    package_name = f"StepONE_v{version}_linux"
    package_dir = version_dir / package_name

    if not (linux_source / "stepone_app").exists():
...
def package_web(project_root, version_dir, version):
    ...
    package_name = f"StepONE_v{version}_web"
...
def package_android(project_root, version_dir, version):
    ...
    apk_name = f"StepONE_v{version}_android.apk"
...
def package_installer(project_root, version_dir, version):
    ...
    installer_source = project_root / "build" / "installer" / f"StepONE_Setup_v{version}.exe"
    installer_name = f"StepONE_v{version}_windows_installer.exe"
...
示例:
  ...
  python package.py 1.0.0 --platforms=macos
  python package.py 1.0.0 --platforms=windows,android,macos
  python package.py 1.0.0 --all-platforms
        """
    )
